            ...
            if model.isPressed {
                if simpleVisuals {
                    ...
                    RoundedRectangle(cornerRadius: 12)
                        ...
                        .hueRotation(hueShift)
                        .shadow(color: model.color, radius: CGFloat(10 + (model.pressure * 30))) // Wide breathing glow
                        .shadow(color: Color.white.opacity(Double(model.pressure)), radius: 2)  // Hot white core on hard press
                }
            }

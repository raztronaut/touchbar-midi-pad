            ...
            HStack(spacing: 0) {
                ...
                VStack(spacing: 20) {
                    ...
                    Button(action: {
                        ...
                        ZStack {
                            Circle()
                                .fill(viewModel.isPoweredOn ? AnyShapeStyle(DesignSystem.Gradients.reactorCore) : AnyShapeStyle(Color.gray.opacity(0.3)))
                                .frame(width: 44, height: 44)
                                .shadow(color: viewModel.isPoweredOn ? .red : .clear, radius: 10)
                                .overlay(
                                    Circle()
                ...
                .padding(.top, 10) // Small padding to push content down slightly from very top edge if needed, but background must go all way up
                .tahoeBackground(material: .sidebar) // Sidebar Transparency
                .ignoresSafeArea(.all, edges: .top) // CRITICAL: This allows the sidebar material to go behind traffic lights
                .overlay(
                    Rectangle()
                        .frame(width: 1)
                ...
                GeometryReader { geo in
                    ...
                    LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 14), count: 4), spacing: 14) {
                        ForEach(sortedPads) { pad in
                            PadView(model: pad, simpleVisuals: viewModel.useSimpleVisuals)
...
struct SettingsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Toggle("Simple Visuals", isOn: $viewModel.useSimpleVisuals)
                .toggleStyle(.switch)
